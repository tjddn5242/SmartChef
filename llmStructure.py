...
def gptOutput(user_need, ingredients, disease):
    ...
    1. [재료]와 그 재료로 만들 수 있는 [레시피], 그리고 [현재 땡기는 음식] 정보를 기반으로 '레시피(recipes)'를 만드세요. 레시피는 총 3개를 만들어야 하며, 각 레시피에는 '요리 제목(name)', '추가 구비 재료(additional_ingredients)', '전체 필요 재료(all_ingredients)', '요리 시간(cooking_time)', '요리 단계(step)'이 필요합니다.
    2. [레시피]를 참고해서 '요리 제목', '추가 구비 재료', '전체 필요 재료', '요리 시간', '요리 단계'를 생성하세요. '요리 제목'은 요리의 제목을 의미하고, '추가 구비 재료'는 [재료]에는 없지만 요리에 필요한 재료를 의미하고, '전체 필요 재료'는 요리에 필요한 모든 재료를 의미하고, '요리 시간'은 요리에 필요한 시간을 의미합니다. '요리 단계'는 3단계로 나누어서(줄넘김으로 출력) 초보자도 이해하기 쉽게 설명하세요.
    3. 요리 초보자의 [몸상태(질병) 정보]와 [질병에 따른 건강정보]를 토대로 '쉐프의 한 마디(chefTip)'를 출력하세요. '쉐프의 한 마디'는 전체 요리에 대해 건강 관점에서 음식에 대한 설명을 해줘야합니다. '쉐프의 한 마디'는 최대한 자세하고 길게 설명해주고, 주의식품에 대해 강조해서 설명해주세요
