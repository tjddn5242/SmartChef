...
def gptOutput(user_need, ingredients, disease):
    ...
    2. [레시피]를 참고해서 '요리 제목', '추가 구비 재료', '전체 필요 재료', '요리 시간', '요리 단계'를 생성하세요. '요리 제목'은 요리의 제목을 의미하고, '추가 구비 재료'는 [재료]에는 없지만 요리에 필요한 재료를 의미하고, '전체 필요 재료'는 요리에 필요한 모든 재료를 의미하고, '요리 시간'은 요리에 필요한 시간을 의미합니다. '요리 단계'는 3단계로 나누어서(줄넘김으로 출력) 초보자도 이해하기 쉽게 설명하세요.  '건강 점수'는 [질병에 따른 건강정보]를 토대로 계산해주세요. '전체 필요 재료'에 [질병에 따른 건강정보]에 있는 주의식품이 많으면 점수가 깎이고, 권장식품이 많으면 점수가 올라갑니다. 그리고, 예시로 4점은 ★★★★☆ 같이 표현합니다.
