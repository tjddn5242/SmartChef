...
def gptOutput(user_need, ingredients, disease):
    ...
    1. [재료]와 그 재료로 만들 수 있는 [레시피], 그리고 [현재 땡기는 음식] 정보를 기반으로 '레시피(recipes)'를 만드세요. 레시피는 총 3개를 만들어야 하며, 각 레시피에는 '요리 제목(name, english_name)', '추가 구비 재료(additional_ingredients)', '전체 필요 재료(all_ingredients)', '요리 시간(cooking_time)', '요리 단계(step)'이 필요합니다.
